-- Phase 8: Effect Engine
-- Updates select_modification (initial mod state), submit_answer (full scoring with
-- profiles / modifications / anomalies), and advance_room_phase (anomaly assignment).

-- ─────────────────────────────────────────────────────────────────────────────
-- Add settings column to room_players (used by Katalizator passive tracking)
-- ─────────────────────────────────────────────────────────────────────────────
alter table room_players
  add column if not exists settings jsonb not null default '{}';

-- ─────────────────────────────────────────────────────────────────────────────
-- select_modification — sets initial remaining_questions / uses_remaining / state
-- Also handles Katalizator passive (sets katalizator_pending flag) and
-- Przekaźnik passive (first time-based mod gets +1 question).
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  case p_modification_id
    ...
    when 'opozniona-reakcja'       then v_init_rq := null;  -- permanent, tracked via draft_stage
  ...
  -- Determine if this is a time-based (questionsRemaining) modification
  ...
  -- Record selection in modification options
  update room_modification_options
  set selected_modification_id = p_modification_id,
      selected_at = now()
  where room_id = p_room_id and user_id = auth.uid() and draft_stage = p_draft_stage;

  -- Record in player_modifications with initial state
  ...
  -- Katalizator passive: set pending flag on player so next correct answer gives +40
  ...
  -- Auto-advance when all players have selected
...
-- ─────────────────────────────────────────────────────────────────────────────
-- submit_answer — Phase 8: applies Profile, Modification and Anomaly effects
-- Mirrors the TypeScript effectEngine.ts computeScore() function exactly.
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  if v_profile_id = 'chronotyp' then
    ...
    -- Note: active (+4s) is managed by client; cannot be verified server-side here
  end if;
  ...
  if v_petla_next_bonus then
    ...
    -- Clear nextQBonus flag now that it has been applied
  ...
  if v_is_correct then
    -- Horyzont passive (+20%) / active (+40% total) in q10–12
    -- Note: active usage tracked client-side; server uses passive by default
    ...
    begin
      ...
      if v_has_petla and v_petla_uses > 0 and not v_shielded then
        ...
        -- Consume use and set nextQBonus
        update player_modifications
        set uses_remaining = 0,
...
-- ─────────────────────────────────────────────────────────────────────────────
-- advance_room_phase — Phase 8 update:
-- Assigns a deterministic anomaly to room_questions for q4 and q8
-- when advancing to those questions (used by Phase 9 banner display).
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  if v_room.status = 'question' then
    ...
    if v_room.current_question_index >= 12 then
      ...
      -- Assign anomaly to q4 and q8 (deterministic from room seed)
      if v_next_idx in (4, 8) then
        select array_agg(id order by id) into v_anomaly_ids
        from anomalies where is_enabled = true;

        if v_anomaly_ids is not null and array_length(v_anomaly_ids, 1) > 0 then
          v_seed_float := hashtext(
            v_room.seed || ':anomaly:' || v_next_idx::text
          )::float8 / 2147483648.0;
          ...
          where room_id = p_room_id and question_index = v_next_idx;
        end if;
      end if;
      ...
      set
        ...
        active_anomaly_id         = case when v_next_idx in (4, 8) then v_anomaly_id else null end,
        current_phase_started_at  = now(),
        current_phase_duration_ms = 15000
      where id = p_room_id;
    end if;
